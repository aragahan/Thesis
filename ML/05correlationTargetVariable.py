...
#Based on our Correlation Heatmap, we can see there are no features that have
#A correlation of 0.5 or higher to the target variable Diabetic.
#There exists only 3 variables that have relatively near correlation values, that is:
#Systolic blood pressure = 0.16
#Body temperature = 0.12
#Sweating = 0.14
#For the sake of the experiment, we decide to keep the features that have the greatest correlation coefficients 
#to the target variable
#As a result, we keep the 3 features mentioned earlier (as well as the target variable) and drop the rest
df = df[['Systolic Blood Pressure', 'Body Temperature', 'Sweating', 'Diabetic']]



#Check the dataset again to see if only the 3 features remain alongside the target variable
#print(df.head())
...
classifier.fit(x_train, y_train) #x_train_smote here is a dataFrame
...
logreg.fit(x_train, y_train) #x_train_smote here is a dataFrame
...
model_shap = rf.fit(x_train_rf, y_train)
