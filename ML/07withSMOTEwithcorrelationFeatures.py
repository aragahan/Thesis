...
#Based on our Correlation Heatmap, we can see there is a high correlation
#Between sweating and shivering. However, we want to minimize correlation between features
#Upon analysis, it can be seen that there seems to have more values 
#That are of greater correlation between sweating and the other features
#When compared to shivering. Due to this, we will decide to drop the Sweating feature
#And move on with 8 features in the dataset.
df = df.drop(['Sweating'], axis = 1)


#Check the dataset again to see if the Sweating feature was dropped
#print(df.head())
...
classifier.fit(x_train_smote, y_train_smote.ravel()) #x_train_smote here is a dataFrame
...
logreg.fit(x_train_smote, y_train_smote.ravel()) #x_train_smote here is a dataFrame
...
x_train_2 = scaler.fit_transform(x_train_smote)
...
knn.fit(x_train_2, y_train_smote.ravel()) #x_train_smote here is a dataFrame
...
scaling = MinMaxScaler(feature_range=(-1,1)).fit(x_train_smote)
x_train_svm = scaling.transform(x_train_smote)
x_test_svm = scaling.transform(x_test)

#Train the model using the training sets
clf.fit(x_train_svm, y_train_smote.ravel())
