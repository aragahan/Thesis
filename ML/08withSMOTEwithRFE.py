...
#We will use a Logistic regression model to evaluate the features during the recursive search
rfecv_model = LogisticRegression(max_iter=7600)
...
#Optimal features = 7
#'Blood Glucose', 'Diastolic Blood Pressure', 'Systolic Blood Pressure',
# 'Body Temperature', 'SPO2', 'Sweating', 'Shivering'
#Age and Heart Rate were eliminated

#Now that we have the features that we need, we can reduce the dataset to these reduced features
target = df['Diabetic']
data = df.drop(['Diabetic','Age','Heart Rate'], axis = 1)
x_train, x_test, y_train, y_test = train_test_split(data, target, test_size=0.2)
